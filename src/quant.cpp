...
QuantizedMatrix QuantizedMatrix::quantize(const Matrix &source,
                                          QuantType type,
                                          size_t block_size) {
    QuantizedMatrix result(source.rows(), source.cols(), block_size, type);
    const int limit = type == QuantType::int8 ? 127 : 7;
    for (size_t block = 0; block < result.blocks(); block++) {
        const size_t begin = block * block_size;
        const size_t end = std::min(begin + block_size, source.size());
        float maximum = 0.0f;
        for (size_t index = begin; index < end; index++) {
            if (!std::isfinite(source.data()[index]))
                throw std::invalid_argument(
                    "quantize: non-finite source weight");
            maximum = std::max(maximum, std::fabs(source.data()[index]));
        }
        ...
        for (size_t index = begin; index < end; index++) {
            int quantized =
                int(std::lround(source.data()[index] / stored_scale));
...
void matvec_quantized(const QuantizedMatrix &matrix, const float *vector,
    ...
    for (size_t row = 0; row < matrix.rows(); row++) {
        ...
        for (size_t column = 0; column < matrix.cols(); column++)
            sum += double(matrix.value(row, column)) *
